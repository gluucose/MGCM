...
# Define parameter combinations for module detection
min_module_sizes <- c(9, 11, 13, 15, 17)
merge_heights <- c(0.15, 0.2, 0.25, 0.3, 0.35)
...
for (size in min_module_sizes) {
  for (height in merge_heights) {
    ...
    net <- blockwiseModules(
      ...
      mergeCutHeight = height,     # Module merging threshold
    ...
    # Record module sizes for current parameters
...
# Plot module distribution by parameters
library(ggplot2)
ggplot(results, aes(x = interaction(ModuleSize, MergeHeight), y = GeneCount, fill = Module)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(
    title = "Module Distribution by Parameters",
    x = "Module Size and Merge Height",
    y = "Gene Count"
  ) +
  theme_minimal() +
